...
export class GitWorktreeManager implements WorktreeManager {
  ...
  async createWorktree(task: Task): Promise<Result<WorktreeInfo>> {
    ...
    const branchName = sanitizeBranchName(task.branchName || `delegate/task-${task.id.slice(0, 8)}`);
  ...
  async completeTask(task: Task, info: WorktreeInfo): Promise<Result<CompletionResult>> {
    ...
    return err(new DelegateError(ErrorCode.INVALID_INPUT, `Unknown merge strategy: ${task.mergeStrategy}`));
  }
      ...
      if (this.githubIntegration) {
        const prResult = await this.githubIntegration.createPR({
          ...
          body: task.prBody
            ...
            : `Automated changes from Delegate task ${task.id}
...
**Base**: ${info.baseBranch}`,
  ...
  private async isWorktreeSafeToRemove(worktreePath: string): Promise<Result<void>> {
    try {
      ...
      if (hasUnpushedChanges) {
        return err(
          new DelegateError(ErrorCode.SYSTEM_ERROR, 'Worktree has unpushed changes - would lose developer work'),
        );
      }
  ...
  async getWorktreeStatus(taskId: TaskId): Promise<Result<WorktreeStatus>> {
    const info = this.activeWorktrees.get(taskId);
    if (!info) {
      return err(new DelegateError(ErrorCode.TASK_NOT_FOUND, `Worktree not found for task ${taskId}`));
    }
  ...
  async cleanup(force = false): Promise<Result<void>> {
    ...
    if (errors.length > 0) {
      ...
      return err(new DelegateError(ErrorCode.SYSTEM_ERROR, `Failed to cleanup some worktrees: ${errors.join(', ')}`));
    }
