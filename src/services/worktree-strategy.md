...
#### Patch
- Generate patch file in `.delegate-patches/`
...
delegate delegate "refactor database layer" \
  --use-worktree \
  --create-branch \
  --merge-strategy manual

# Automated testing with auto-merge
delegate delegate "update dependencies" \
  --use-worktree \
  --create-branch \
  --auto-commit \
  --merge-strategy auto
...
delegate delegate "implement new feature" \
  --use-worktree \
  --create-branch \
  --push-to-remote \
  --merge-strategy pr \
...
delegate delegate "experimental optimization" \
  --use-worktree \
...
git branch --list "delegate/task-*"

// Clean up old worktrees
git worktree prune
git branch -d delegate/task-abc123
