...
export class GitHubIntegration {
  ...
  async createPR(options: PROptions): Promise<Result<string>> {
    ...
    try {
      ...
      try {
        const args = ['pr', 'create', '--title', options.title, '--body-file', tmpFile, '--base', options.baseBranch];
        ...
        const prUrl = await retryWithBackoff(() => this.executeGhCommand(args, { cwd: options.cwd }), {
          maxRetries: 3,
          initialDelay: 2000,
          maxDelay: 10000,
          logger: this.logger,
  ...
  async getPRStatus(prNumber: string): Promise<Result<PRStatus>> {
    ...
    try {
      ...
      const stdout = await retryWithBackoff(
        () => this.executeGhCommand(['pr', 'view', prNumber, '--json', 'state,mergeable,url']),
        {
          maxRetries: 3,
          initialDelay: 1000,
          logger: this.logger,
  ...
  async mergePR(prNumber: string, method: 'merge' | 'squash' | 'rebase' = 'merge'): Promise<Result<void>> {
    ...
    try {
      ...
      await this.executeGhCommand(['pr', 'merge', prNumber, `--${method}`, '--delete-branch']);
